  ...
  flake.modules.nixos.base =
    { pkgs, ...}:
    {
      ...
      nix.settings = {
        ...
        # Размер внутреннего буфера загрузки в байтах. Загрузка приостанавливается, когда обработка
        # данных из буфера не поспевает за загрузкой. Значение по умолчанию — 64 Mb
        download-buffer-size = 128 * 1024 * 1024;  # 128 Mb
        ...
        # Количество секунд, в течение которых загруженный tar-архив считается актуальным
	      tarball-ttl = 24 * 60 * 60;  # 24 часа, по-умолчанию - 1 час
